...
- `scp filename.py username@ssh.rc.byu.edu:/path/to/destination`: Copy a file from your local machine to the supercomputer
...
- `source /etc/profile`: Load the system-wide environment settings and paths
    - Run this first every time you log in so that tools like `mamba` and other modules are available
...
- `git add`: Stage files that have been changed so they are included in the next commit
    - Use `--all` to add all changes (excluding untracked files, e.g. those in the `.gitignore`), or `filename.py` to add a specific file

- `git commit -m "example message"`: Take staged changes and commit them, with a comment explaining what changed in the commit. 
    - You can also run `git commit -am "example message"` to stage all modified tracked files and commit them at the same time 
...
- `:w`: Save the current file (write)
    - Use `:w filename.py` to save as a specific filename
...
- `python src/main.py fit --config config.yaml`: Train a model using settings from a config file
...
- `python src/main.py preprocess --config config_preprocess.yaml`: Preprocess a dataset without training
