...
class RequeueCallback[RawT, ProcessedT, BatchT, ModelOutputT](Callback[RawT, ProcessedT, BatchT, ModelOutputT]):
    def __init__(self, requeue_signal: int = signal.SIGHUP) -> None:
        ...
        self.requeue_global_step: int | None = None
    ...
    def _load_requeue_checkpoint_if_present(
        ...
        try:
            with open(path, "r") as f:
                ...
                self.requeue_global_step = state["global_step"]
    ...
    def _create_requeue_handoff(self, training_run_id: str):
        path = requeue_handoff_path()
        handoff: RequeueHandoff = {"previous_training_run_id": training_run_id}
